...
def handle_backup(config, backup_name_arg, stagger_time, enable_md5_checks_flag, mode, skip_snapshot_creation):
    ...
    with Storage(config=config.storage) as storage:
        try:
            ...
            if node_backup.exists() and not skip_snapshot_creation:
            ...
            info = start_backup(storage, node_backup, cassandra, differential_mode, stagger_time, start, mode,
                                enable_md5_checks_flag, backup_name, config, monitoring, skip_snapshot_creation)
...
def start_backup(storage, node_backup, cassandra, differential_mode, stagger_time, start, mode,
    ...
    if skip_snapshot_creation and not cassandra.snapshot_exists(backup_name):
        raise IOError("Error: Snapshot {} does not exist and skipping creation. Please create the snapshot first.".format(backup_name))
    ...
    num_files, num_replaced, num_kept = do_backup(
        cassandra, node_backup, storage, enable_md5, backup_name, skip_snapshot_creation
    )
...
def do_backup(cassandra, node_backup, storage, enable_md5_checks, backup_name, skip_snapshot_creation):
    # cassandra.snapshot_exists_on_node(backup_name)
    if skip_snapshot_creation:
        logging.debug('Skipping snapshot creation')
        logging.debug("Getting snapshot")
        snapshot = cassandra.get_snapshot(backup_name, delete_on_exit=False)
    else:
